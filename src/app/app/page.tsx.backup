...
type VerificationStep = 'form' | 'email-otp' | 'phone-otp' | 'complete';
...
export default function AuthPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      if (activeTab === 'signup') {
        // For signup, start dual verification flow
        setVerificationStep('email-otp');
        setEmailOtpTimer(60);
        setToast({ type: 'info', message: `OTP sent to ${email}` });
      } else {
        // For signin, just show single email OTP
        setVerificationStep('email-otp');
        setEmailOtpTimer(60);
  ...
  const handleEmailOTPPaste = (e: React.ClipboardEvent) => {
    ...
    setEmailOtp(newOtp);
    emailOtpInputRefs.current[Math.min(pastedData.length, 5)]?.focus();
  };
  ...
  const handlePhoneOTPPaste = (e: React.ClipboardEvent) => {
    ...
    const newOtp = pastedData.split('').concat(Array(6).fill('')).slice(0, 6);
    setPhoneOtp(newOtp);
    phoneOtpInputRefs.current[Math.min(pastedData.length, 5)]?.focus();
  };

  const handleVerifyEmailOTP = () => {
    const otpValue = emailOtp.join('');
    if (otpValue.length !== 6) {
      setToast({ type: 'error', message: 'Please enter complete OTP' });
      return;
    }
    ...
    setTimeout(() => {
      ...
      // Move to phone verification for signup
      if (activeTab === 'signup') {
        setTimeout(() => {
          setVerificationStep('phone-otp');
          setPhoneOtpTimer(60);
          setToast({ type: 'info', message: `OTP sent to +91 ${phone}` });
        }, 1000);
      } else {
        // For signin, complete after email verification
        setTimeout(() => {
  ...
  const handleVerifyPhoneOTP = () => {
    ...
    setTimeout(() => {
      ...
      trackEvent('phone_verified', { phone });
    }, 1500);
  };
  ...
  const handleResendEmailOTP = () => {
    ...
    setEmailOtpTimer(60);
    setToast({ type: 'success', message: 'New OTP sent to email' });
    trackEvent('otp_resend', { type: 'email' });
  };

  const handleResendPhoneOTP = () => {
    if (phoneOtpTimer > 0) return;
    setPhoneOtpTimer(60);
    setToast({ type: 'success', message: 'New OTP sent to phone' });
    trackEvent('otp_resend', { type: 'phone' });
  };
        ...
        <div className="auth-layout">
          <div className="auth-content">
            <div className="otp-container">
              ...
              <div className="otp-inputs" onPaste={handleEmailOTPPaste}>
              ...
              <button onClick={() => setVerificationStep('form')} className="otp-back">
            ...
            <div className="otp-container">
              <div className="otp-header">
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
                  ...
                  <span style={{ color: '#10b981', fontWeight: '600' }}>Email Verified</span>
                </div>
              ...
              <div className="otp-inputs" onPaste={handlePhoneOTPPaste}>
              ...
              <div style={{ textAlign: 'center', padding: '2rem 0' }}>
                <div style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'center' }}>
                  <div style={{
                    ...
                    <svg width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3">
                      <polyline points="20 6 9 17 4 12" />
                    </svg>
                  </div>
                </div>
                <h2 style={{ fontSize: '2rem', fontWeight: '700', color: '#0F172A', marginBottom: '1rem' }}>
                  {activeTab === 'signup' ? 'Account Created Successfully!' : 'Signed In Successfully!'}
                </h2>
                <p style={{ fontSize: '1rem', color: '#64748b', marginBottom: '2rem', lineHeight: '1.6' }}>
                  {activeTab === 'signup' 
                    ? `Welcome ${name}! Your email and phone have been verified.` 
                  ...
                  <div style={{ 
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <span style={{ color: '#0F172A', fontWeight: '500' }}>Email: {email}</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2.5">
                          <polyline points="20 6 9 17 4 12" />
                        </svg>
                        <span style={{ color: '#0F172A', fontWeight: '500' }}>Phone: +91 {phone}</span>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <button
                  onClick={() => {
                    ...
                    window.location.href = 'http://app.localhost:3000/dashboard';
                  }}
                  className="btn-primary"
